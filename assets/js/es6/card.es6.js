...
class CardValidator {
    static validateBin(card, num) {
        return card.regexpBin.test(Number(num));
    }

    static validateCard(card, num) {
        return card.regexpFull.test(Number(num));
    }

    static checkFlag(num) {
...
$(document).ready(function() {
    const cardInstance = $("[data-card-form]");
    const cardContainer = $("[data-card-container]");

    function updateDate() {
        const month = ("0" + cardInstance.find("[data-card-month]").val()).slice(-2);
        const year = cardInstance.find("[data-card-year]").val() % 1000;
    ...
    cardInstance.find("[data-card-number]").on("input", function() {
        const flag = CardValidator.checkFlag($(this).cleanVal());
        cardContainer.find(".flag").attr("src", flag.image || FLAGS.default.image);

        const value = $(this).mask(flag.mask || FLAGS.default.mask).masked($(this).val());
