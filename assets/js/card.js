var CardValidate = {
    ...
    checkFlag: function(num) {
		for (var flag in CardValidate.flags) {
			if (CardValidate.validateBin(CardValidate.flags[flag], num)) {
				return CardValidate.flags[flag];
			}
		}

		return CardValidate.flags.default;
	},

	checkFullCard: function(num) {
		for (var flag in CardValidate.flags) {
			if (CardValidate.validateCard(CardValidate.flags[flag], num)) {
				return true;
			}
		}
...
$(document).ready(function() {
    ...
    cardInstance.find("[data-card-number]").mask(CardValidate.flags.default.mask);
    cardInstance.find("[data-card-code]").mask(CardValidate.flags.default.maskCvv, {translation:  {Z: {pattern: /[0-9]/, optional: true}}});
    updateDate();
    ...
    cardInstance.find("[data-card-number]").on("input", function() {
        var flag = CardValidate.checkFlag($(this).cleanVal());
        cardContainer.find(".flag").attr("src", flag.image || CardValidate.flags.default.image);

        var value = $(this).mask(flag.mask || CardValidate.flags.default.mask).masked($(this).val());
        cardContainer.find("#card-number-p").text(value || "0123 4567 8910 1112");

        cardInstance.find("[data-card-code]").mask(flag.maskCvv || CardValidate.flags.default.maskCvv, {translation:  {Z: {pattern: /[0-9]/, optional: true}}});
    });
