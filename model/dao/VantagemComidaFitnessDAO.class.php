<?php
    ...
    class VantagemComidaFitnessDAO {
        public static function listar() {
            ...
            if ($stmt->execute()) {
                while ($rs = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $itens[] = new VantagemComidaFitness($rs);
                }
            }
        ...
        public static function selecionar($id) {
            ...
            if ($stmt->execute()) {
                if ($rs = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $item = new VantagemComidaFitness($rs);
                }
            }
        ...
        public static function inserir($item) {
            $conn = Database::getConnection();
            $stmt = $conn->prepare("INSERT INTO tbl_vantagem_comida_fitness (id_funcionario, titulo, texto, ativo, data) VALUES (?, ?, ?, ?, NOW())");
            ...
            $resultado = null;
            if ($stmt->execute()) {
                $resultado = VantagemComidaFitnessDAO::selecionar($conn->lastInsertId());
            }
        ...
        public static function atualizar($id, $item) {
            $conn = Database::getConnection();
            $stmt = $conn->prepare("UPDATE tbl_vantagem_comida_fitness SET titulo = ?, texto = ?, ativo = ? WHERE id = ?");
        ...
        public static function ativar($id) {
            $conn = Database::getConnection();
            $stmt = $conn->prepare("UPDATE tbl_vantagem_comida_fitness SET ativo = !ativo WHERE id = ?");
            $stmt->bindParam(1, $id);
            $resultado = $stmt->execute() ? $stmt->rowCount() : -1;
            $conn = null;
            return $resultado;
        ...
        public static function excluir($id) {
            $conn = Database::getConnection();
            $stmt = $conn->prepare("DELETE FROM tbl_vantagem_comida_fitness WHERE id = ?");
            $stmt->bindParam(1, $id);
            $resultado = $stmt->execute() ? $stmt->rowCount() : -1;
            $conn = null;
            return $resultado;
